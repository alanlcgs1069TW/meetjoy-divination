...
// 五鼠遁日起時法：甲己→甲子起, 乙庚→丙子起, 丙辛→戊子起, 丁壬→庚子起, 戊癸→壬子起
// （與 lunarConverter.ts 的本命時干同源，勿改）
...
/**
 * 國曆 Date 的「小時」→ 時辰 index 0–11（子=0）。
 * 子時橫跨午夜（23:00–01:00），故 23 時亦歸 0；晚子時的「日期 +1」由呼叫端處理。
 */
export function timeIndexOfDate(d: Date): number {
  return Math.floor(((d.getHours() + 1) % 24) / 2);
}

/** 時辰 index 0–11 → 該時辰的代表小時（子=0、丑=1、寅=3 … 亥=21），供組 queryDate 用。 */
export function hourOfTimeIndex(ti: number): number {
  const i = ((ti % 12) + 12) % 12;
  return i === 0 ? 0 : i * 2 - 1;
}

/** 流時干支字串：日干（五鼠遁）起時干 + 時支。 */
export function hourlyGanZhi(queryDate: Date, timeIndex: number): string {
  const { stemIdx: dayStemIdx } = getDayGanzhi(queryDate);
  const ti = ((timeIndex % 12) + 12) % 12;
  return STEMS[(HOUR_STEM_START[dayStemIdx % 5] + ti) % 10] + BRANCHES[ti];
}
 ...
 * 建立流時 Horoscope 資料
 *
 * 斗君鏈的最末節（學會版未教到流時，依既有斗君邏輯延伸）：
 *   流時命宮 = 流日命宮 + 時辰序（子=0 起，順行，一時辰一宮）
 * 時干用五鼠遁（日干起時干），與本命時干同源。
 * 晚子時（23:00–24:00）由呼叫端把 queryDate +1 天、時辰改早子後傳入，
 * 與 createAstrolabe 的本命處理一致（純排盤規則，不代表實際日期是隔日）。
 */
export function buildHourlyHoroscope(
  queryDate: Date,
  palaces: PalaceRef[],
): {
  palaceIndex: number;
  ...
  const timeIndex = timeIndexOfDate(queryDate);

  // 時干（五鼠遁）→ 時祿/時羊/時陀 + 流時四化
  const { stemIdx: dayStemIdx } = getDayGanzhi(queryDate);
  const hourStemIdx = (HOUR_STEM_START[dayStemIdx % 5] + timeIndex) % 10;
  ...
  const dailyBranchIdx  = (monthlyBranchIdx + (lunarDay - 1)) % 12;
  const hourlyBranchIdx = (dailyBranchIdx + timeIndex) % 12;

  const hourlyBranch       = BRANCHES[hourlyBranchIdx] as EarthlyBranch;
  const hourlyHeavenlyStem = STEMS[hourStemIdx]        as HeavenlyStem;

  const hourlyPalace     = palaces.find(p => p.earthlyBranch === hourlyBranch);
  const hourlyPalaceIdx  = hourlyPalace?.index ?? 0;
  const hourlyPalaceName = hourlyPalace?.name  ?? '';

  const mutagen = MUTAGEN_TABLE[hourStemIdx];

  // 時祿/時羊/時陀（依時干）
  const out = new Map<number, StarRef[]>();
  const hLuCun    = LU_CUN[hourStemIdx];
  const hQingYang = (hLuCun + 1) % 12;
  const hTuoLuo   = (hLuCun - 1 + 12) % 12;
  placeStar(hLuCun,    '時祿', 'hourly', palaces, out);
  placeStar(hQingYang, '時羊', 'hourly', palaces, out);
  placeStar(hTuoLuo,   '時陀', 'hourly', palaces, out);

  return {
    palaceIndex:   hourlyPalaceIdx,
    palaceName:    hourlyPalaceName,
    heavenlyStem:  hourlyHeavenlyStem,
