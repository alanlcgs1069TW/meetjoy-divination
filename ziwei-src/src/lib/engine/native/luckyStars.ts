...
/**
 * 天馬地支（節氣月支三合驛馬）✅ VERIFIED 2026-05-20（9 筆，對照學會版排盤確認）
 *
 ...
 *   巳酉丑月(1,5,9) → 亥(11)
 *
 * 已對照學會版排盤確認：其天馬位置表與上述三合驛馬規則完全一致
 * （子/辰/申→寅、丑/巳/酉→亥、寅/午/戌→申、卯/未/亥→巳），
 * 且天馬是唯一以「節氣月支」為索引的星曜，不與其他星曜共用同一組索引。
 *
 * 原驗證為 9 筆實際命盤（四馬地各 ≥1 筆、含晚子時案例），
 * 命例資料因隱私已移除；結論為 8/9 相符、1 筆待補完整生日。
 *
 * ⛔ WARNING — 此公式經歷三次嘗試才找到：
 *   2026-05-04 v1：年支起 [2,8,11,5,...] ← 錯
 *   2026-05-18 v2：命宮支起 [8,11,11,5,8,11,11,5,8,11,11,5] ← 看似 verified 但實為循環驗證
 *   2026-05-20 v3：節氣月支三合驛馬 [2,11,8,5,2,11,8,5,2,11,8,5] ✅ 經學會版排盤對照 + 9 筆截圖雙重確認
 *
 * 修改前必須：
 *   1. 確認新規則的學會來源（學會版排盤 / 講義）
