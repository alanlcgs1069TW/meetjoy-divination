 ...
 * Phase 4b（2026-05-04）：新增 16 顆學會版獨有星曜（對照學會版排盤公開資源確認）
...
// 驗證（2026-05-19）：
//   陽男庚干 火六局 寅起 → 順行 ✓（某例：命宮子=胎）
//   陰女癸干 土五局 申起 → 順行 ✓（某例：官祿申=長生，酉=沐浴）
//   陽女庚干 火六局 寅起 → 逆行 ✓（某例：命宮丑=沐浴）
//   陰男己干 土五局 申起 → 逆行 ✓（某例：全盤一致）
...
// 驗證：壬午年 yb=6(午, 寅午戌)→卯(3)，dreamkinin  癸卯夫妻宮: 咸池 ✓
...
// 天廚（年干起，學會版排盤 "no":"year" key=(ys+4)%10）
...
// 驗證：壬(ys=8)→酉(9)，dreamkinin  己酉官祿宮: 天廚 ✓

// 天官（年干起，學會版排盤 "no":"year" key=(ys+4)%10）
...
// 驗證：壬(ys=8)→戌(10)，dreamkinin  庚戌僕役宮: 天官 ✓

// 天福（年干起，學會版排盤 "no":"year" key=(ys+4)%10）
...
// 驗證：壬(ys=8)→午(6)，dreamkinin  丙午父母宮: 天福 ✓
...
export function placeMiscStars(
  ...
  // 蜚廉（年支起，學會版排盤 完整 position table，2026-05-04 修正）
  ...
  // 臺輔（時支起，午宮起子時，順數）
  // 學會版排盤: "begin":"wu","direction":"clockwise","no":"hour"
  const taiFu = (6 + _ti) % 12;
  // 驗證：ti=0(子時)→午(6)，dreamkinin  丙午父母宮: 臺輔 ✓

  // 封誥（時支起，寅宮起子時，順數）
  // 學會版排盤: "begin":"yin","direction":"clockwise"（順行）
  // 正確公式：(2 + ti) % 12
  // ⚠️ 原公式 (2 - ti + 12) % 12 為逆行，ti=0/6 時兩式結果相同故未被發現
  // 驗證（2026-05-19）：ti=2(寅時)→辰(4)，某例官祿辰: 封誥 ✓, 某例夫妻辰: 封誥 ✓
  const fengGao = (2 + _ti) % 12;

  // 天刑（農曆月起，酉宮起正月，順數）
  // 學會版排盤: "begin":"you","direction":"clockwise","no":"month"
  const tianXing = (8 + lunarMonth) % 12;
  // 驗證：lm=4→子(0)，dreamkinin  壬子疾厄宮: 天刑 ✓

  // 天姚（農曆月起，丑宮起正月，順數）
  // 學會版排盤: "begin":"chou","direction":"clockwise","no":"month"
  const tianYao = lunarMonth % 12; // = (1 + lm - 1) % 12 = lm % 12
  // 驗證：lm=4→辰(4)，dreamkinin  甲辰兄弟宮: 天姚 ✓

  // 陰煞（農曆月起，寅宮起正月，逆數 2/月）
  // 學會版排盤: "no":"month", 寅子戌申午辰循環
  const yinSha = YIN_SHA[lunarMonth - 1];
  // 驗證：lm=4→申(8)，dreamkinin  戊申田宅宮: 陰煞 ✓

  // 天廚（年干起）
  const tianChu = TIAN_CHU[yearStemIdx];
  // 驗證：ys=8(壬)→酉(9)，dreamkinin  己酉官祿宮: 天廚 ✓

  // 天哭（年支起，午宮起子年，逆數）
  // 學會版排盤: "begin":"wu","direction":"counterclock","no":"zodiac"
  const tianKu = (6 - yearBranchIdx + 12) % 12;
  // 驗證：yb=6(午)→子(0)，dreamkinin  壬子疾厄宮: 天哭 ✓

  // 天虛（年支起，午宮起子年，順數）
  // 學會版排盤: "begin":"wu","direction":"clockwise","no":"zodiac"
  const tianXu = (6 + yearBranchIdx) % 12;
  // 驗證：yb=6(午)→子(0)，dreamkinin  壬子疾厄宮: 天虛 ✓

  // 天官（年干起）
  const tianGuan = TIAN_GUAN[yearStemIdx];
  // 驗證：ys=8(壬)→戌(10)，dreamkinin  庚戌僕役宮: 天官 ✓

  // 天福（年干起）
  const tianFu2 = TIAN_FU2[yearStemIdx];
  // 驗證：ys=8(壬)→午(6)，dreamkinin  丙午父母宮: 天福 ✓

  // 龍池（年支起，辰宮起子年，順數）
  // 學會版排盤: "begin":"chen","direction":"clockwise","no":"zodiac"
  const longChi = (4 + yearBranchIdx) % 12;
  // 驗證：yb=6(午)→戌(10)，dreamkinin  庚戌僕役宮: 龍池 ✓

  // 鳳閣（年支起，戌宮起子年，逆數）
  // 學會版排盤: "begin":"xu","direction":"counterclock","no":"zodiac"
  const fengGe = (10 - yearBranchIdx + 12) % 12;
  // 驗證：yb=6(午)→辰(4)，dreamkinin  甲辰兄弟宮: 鳳閣 ✓
  ...
  //   陽男庚干→順行（某例）、陰女癸干→順行（某例）、
  //   陽女庚干→逆行（某例）、陰男己干→逆行（某例/某例）
  ...
  // ✅ VERIFIED 2026-05-19（6 筆）：
  //   陽男庚干→順行（某例：命宮子=將軍）、陰女癸干→順行（某例：財帛子=博士起點）
  //   陽女庚干→逆行（某例：全盤一致）、陰男己干→逆行（某例/某例：全盤一致）
