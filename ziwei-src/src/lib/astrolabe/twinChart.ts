...
export function createTwinChart(original: ZiweiChart, birthOrder: 2 | 3 | 4 = 2): ZiweiChart {
  ...
  function twinHoroscope(queryDate: Date = new Date()): ZiweiHoroscope {
    ...
    // 大限 range 是實歲；偵測用實歲 = 農曆流年 − 農曆生年（換年正月初一，與 decadal.ts 同基準）
    const [by, bm, bd] = original.birthInfo.solarDate.split('-').map(Number);
    const birthLunarYear = solarToLunarParts(new Date(by, bm - 1, bd)).year;
    const virtualAge = solarToLunarParts(queryDate).year - birthLunarYear;
