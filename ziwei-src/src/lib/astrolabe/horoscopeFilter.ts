...
import { buildYearlyHoroscope, buildMonthlyHoroscope, buildDailyHoroscope, buildHourlyHoroscope, buildMinorLimitOverlay } from '../engine/native/horoscope';
...
export function buildZiweiHoroscope(
  ...
  // ── 流時（自主計算；時辰取自 queryDate 的小時）────────────────────────────
  const hourly = buildHourlyHoroscope(queryDate, palaceRefs);
  ...
  return {
    ...
    hourly: {
      palaceIndex:   hourly.palaceIndex,
      palaceName:    hourly.palaceName,
      heavenlyStem:  hourly.heavenlyStem as HeavenlyStem,
      earthlyBranch: hourly.earthlyBranch as EarthlyBranch,
      mutagen:       hourly.mutagen as string[],
      stars:         hourly.stars,
    },
  };
}
