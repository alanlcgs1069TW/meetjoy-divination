...
export function YearlyTimeline({ chart, horoscope, selectedYear, onSelect, decadalRange, isChildhood: isChildhoodProp, advMode }: Props) {
  ...
  // 童限：最前面補「0 歲」= 出生當年（農曆生年）。點它 → queryDate 用農曆生年 →
  // 小限落虛歲1（起運宮）、大限顯童限。讓筆記講的「小限一歲位」在盤上可見。
  if (isChildhood) years.unshift(birthYear);
  ...
  return (
    <div className="yearly-timeline">
      {years.map(year => {
        ...
        const isBirthYear = nominalAge === 0; // 0 歲格（出生當年）
        const sb = yearStemBranch(year);
        const pinyin = stemBranchPinyin(sb[0], sb[1]);
        return (
          <div
            key={year}
            className={`yearly-cell${isActive ? ' yearly-active' : ''}${isBirthYear ? ' yearly-birth' : ''}`}
            onClick={() => onSelect(advMode ? year : (isActive ? null : year))}
            title={isBirthYear ? '出生當年（0歲）：小限起運宮＝虛歲1' : '點擊切換流年'}
          >
