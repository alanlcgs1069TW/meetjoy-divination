...
import { HUA_COLORS } from '../lib/huaColors';
...
interface Props {
  ...
  taijiName?: string;
  taijiNameEn?: string;
  feixingHua?: Record<string, number>; // 星名 → 0祿1權2科3忌（點選宮的宮干飛化）
}
...
function starColorClass(type: string, name: string, scope: MutagenScope = 'origin'): string {
  ...
  if (scope === 'hourly')     return 'star-hourly';
...
type MutagenScope = 'origin' | 'decadal' | 'yearly' | 'monthly' | 'daily' | 'hourly' | 'minorLimit';
...
function StarChip({
  ...
  dailyMutagen?: string;
  hourlyMutagen?: string;
  minorLimitMutagen?: string;
  scope?: MutagenScope;
  showOrigin?: boolean;
  feixingHua?: number; // 0祿1權2科3忌：宮干飛化上色（依四化類型配色）
}) {
  ...
  return (
    <span
      className={`star ${colorClass} ${sizeClass}${isFramed ? ' star-framed' : ''}${feixingHua != null ? ' star-feixing star-feixing-hollow' : ''}`}
      style={feixingHua != null ? ({ '--fx-stroke': HUA_COLORS[feixingHua] } as React.CSSProperties) : undefined}
    >
      ...
      {scope === 'origin' && hourlyMutagen && (
        <MutagenBadge char={hourlyMutagen} scope="hourly" />
      )}
      ...
      {scope === 'hourly' && originMutagenChar && (
        <MutagenBadge char={originMutagenChar} scope="hourly" />
      )}
...
export function PalaceCell({ palace, horoscope, isNatalMode = false, isYearlyMode = false, isMinorLimitMode = false, onMinorLimitToggle, style, isClicked, onClick, minorLimitAges = [], advLayers, advFocus, taijiName, taijiNameEn, feixingHua }: Props) {
  ...
  const showHourlyLayer  = pro ? advLayers!.hourly  : false;
  ...
  const activeHighlightClass = pro
    ? (advFocus === 'hourly'  ? (horoscope.hourly.palaceIndex  === palace.index ? 'active-hourly'  : null)
  ...
  // 立太極不覆蓋本命宮名：native-id 一律顯示本命宮名；立太極角色另加一列 taiji-id
  ...
  const hourlyOverlay  = getHourlyOverlay(palace.index, horoscope.hourly.palaceIndex, locale);
  ...
  const hourlyMutagenMap: Record<string, string> = {};
  (horoscope.hourly.mutagen ?? []).forEach((name, i) => {
  ...
  const hourlyStars: StarInfo[] =
    horoscope.hourly.stars.find(g => g.palaceIndex === palace.index)?.stars ?? [];
  ...
  const hasOverlay = (showDecadalLayer && decadalStars.length > 0)
    ...
    || (showHourlyLayer       && hourlyStars.length      > 0)
  ...
  const originMutagenProps = (name: string) => ({
    ...
    hourlyMutagen:     showHourlyLayer     ? hourlyMutagenMap[name]     : undefined,
      ...
      <div className="star-content" ref={starContentRef}>
        ...
        <div className="star-group">
          {orderMajorForDisplay(palace.majorStars ?? []).map((star, i) => (
            <StarChip key={i} star={star} scope="origin" showOrigin={showOriginMutagen} feixingHua={feixingHua?.[star.name]} {...originMutagenProps(star.name)} />
          ))}
        </div>
        ...
        {hasOverlay && (
          <div className="star-group overlay-combined">
            ...
            {showHourlyLayer && hourlyStars.map((star, i) => (
              <StarChip key={`h${i}`} star={star} scope="hourly" />
            ))}
      ...
      <div className="palace-bottom">
        <div className="palace-blc">
          ...
          <div className="palace-bl-row">
            ...
            {isNatalMode && (
              <span className={`${ageCls} palace-age-center`}>{palace.decadal.range[0]}-{palace.decadal.range[1]}</span>
            )}
          </div>
        </div>
        <div className="palace-br">
          <div className="name-stem">
            <div className="name-col">
              {taijiName && (
                <div className="id-row taiji-id">
                  <span className="id-zh">{taijiName}</span>
                  <span className="id-en">{taijiNameEn}</span>
                </div>
              )}
              {isNatalMode && (
                <span className={`${ageCls} palace-age-top`}>{palace.decadal.range[0]}-{palace.decadal.range[1]}</span>
              )}
              {showHourlyLayer && (
                <div className="id-row hourly-id">
                  <span className="id-zh">
                    {hourlyOverlay.abbr
                      ? <><span className="ov-full">{hourlyOverlay.primary}</span><span className="ov-abbr">{hourlyOverlay.abbr}</span></>
                      : hourlyOverlay.primary}
                  </span>
                  <span className="id-en">{hourlyOverlay.secondary}</span>
                </div>
              )}
              {showDailyLayer && (
