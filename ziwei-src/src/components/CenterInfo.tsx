...
export function CenterInfo({ chart, horoscope, isNatalMode, onReset, multiBirthOrder, onPrevTime, onNextTime, isRectified, childhoodOverride, advMode, onAdvToggle, feixingShow = true, onToggleFeixing, zihuaShow = true, onToggleZihua, notes, onSaveNotes, chartId, alias, onSaveAlias }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    const apply = () => {
      el.style.transform = '';
      // transform 不影響 layout，故此時量到的是自然尺寸
      const availH = box.clientHeight - 2, availW = box.clientWidth - 2;
      const natH = el.offsetHeight, natW = el.offsetWidth;
      if (natH <= 0 || natW <= 0) return;
      // 容忍 1px：僅在確實溢出時才縮放，避免對剛好塞得下的卡片套用 scale(0.99x) 讓文字變糊
      if (natH <= availH + 1 && natW <= availW + 1) return;
      const k = Math.min(availH / natH, availW / natW);
  ...
  return (
    <div className="center-info">
      ...
      <div className="center-info-fit" ref={fitRef}>
      ...
      {isEn ? (
        <div className="center-gender">
          <span className={`center-gender-en yy-${yinYang === '陽' ? 'yang' : 'yin'}`}>{yinYangEn}</span>
          {' '}
          <span className={`center-gender-en g-${birthInfo.gender}`}>{genderEn}</span>
        </div>
      ) : (
        <div className="center-gender">
          <span className="center-gender-zh">
            <span className={`yy-${yinYang === '陽' ? 'yang' : 'yin'}`}>{yinYang}</span>
            <span className={`g-${birthInfo.gender}`}>{genderChar}</span>
          </span>
      ...
      <div className="center-feixing-legend" title={useEnLabels ? 'Palace-stem four transformations' : '宮干四化'}>
        <button
          className={`fx-lg-toggle${feixingShow ? ' on' : ''}`}
          onClick={onToggleFeixing}
          title={useEnLabels ? 'Toggle flying-transformation star colouring' : '飛化上色 開/關（宮干四化飛入他宮上色）'}
        >{useEnLabels ? 'Fēihuà' : '飛化'}</button>
        <button
          className={`fx-lg-toggle${zihuaShow ? ' on' : ''}`}
          onClick={onToggleZihua}
          title={useEnLabels ? 'Toggle self-transformation arrows (centrifugal/centripetal)' : '自化箭頭 開/關（離心=自化出、向心=自化入）'}
        >{useEnLabels ? 'Zìhuà' : '自化'}</button>
        {(useEnLabels ? ['Lù', 'Quán', 'Kē', 'Jì'] : ['祿', '權', '科', '忌']).map((label, i) => (
          <span key={i} className="fx-lg" style={{ color: HUA_COLORS[i] }}>{label}</span>
        ))}
      </div>
