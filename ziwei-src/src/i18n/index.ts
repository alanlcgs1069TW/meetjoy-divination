...
export function getHourlyOverlay(palaceIdx: number, hourlyStartIdx: number, locale: Locale = 'zh-TW'): { primary: string; secondary: string; abbr?: string } {
  // 流時宮位：CCW 方向從流時命宮起，前綴「時」（時命/時財…）
  const offset = (palaceIdx - hourlyStartIdx + 12) % 12;
  const shortArray = getPalaceShortArray(locale);
  if (locale === 'en') return { primary: shortArray[offset], secondary: palaceShortPinyin(offset), abbr: PALACE_ABBR_EN[offset] };
  return { primary: '時' + shortArray[offset], secondary: palaceShortPinyin(offset) };
}
...
/** Convert Chinese lunar date string to Arabic numerals, e.g. 二〇〇〇年正月初一 →  */
