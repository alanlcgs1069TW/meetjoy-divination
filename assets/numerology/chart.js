(() => {
  ...
  function renderTriangle(root, lifeNumber, flowYear) {
    ...
    root.querySelector('[data-time-triangle]').innerHTML = `<svg viewBox="-25 -5 260 200" role="img" aria-label="主命數 ${lifeNumber} 的九年循環三角盤"><path d="M36 160V12L188 160Z"/><path d="M36 160H188"/><text x="18" y="165" style="text-anchor:end">命盤柱</text>${points.map((point, index) => `<g class="${sequence[index] === flowYear ? 'is-current' : ''}${[0, 3, 6].includes(index) ? ' is-vertex' : ''}"><circle cx="${point[0]}" cy="${point[1]}" r="${[0, 3, 6].includes(index) ? 15 : 13}"/><text class="triangle-num" x="${point[0]}" y="${point[1] + 4}">${sequence[index]}</text><text class="triangle-word" x="${point[0]}" y="${point[1] + 24}" font-size="9" font-weight="bold">${starWords[index]}</text></g>`).join('')}</svg><p>數字由太陽盤主命數在左下直角「名」起算、順時針鋪滿三角形（名財官利交敗衰煞絕）；三頂點為同組數（147／258／369）。</p>`;
  }
